...
@WebServlet(name = "FollowItemServlet", urlPatterns = {"/FollowItemServlet"})
public class FollowItemServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        follow.setListingId(listing.getId());
        follow.setUserId(user.getUserid());
        
        FollowItemBean fbean = new FollowItemBean();      
        int i = fbean.save(follow);
        
         if(i>0)
        {
            session.setAttribute("followItem", 1);
            
        }
        else
        {
            session.setAttribute("followItem", 0);
        }
        
        response.sendRedirect("/bidapp/item.jsp");
